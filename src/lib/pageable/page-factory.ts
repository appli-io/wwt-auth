...
export class PageFactory<TEntity extends object, TOutput extends object = TEntity, TPage = Page<TOutput>> {
  ...
  async create(): Promise<TPage> {
    ...
    const applyRelation = (relation: Relation) => {
      const joinType: JoinType = getJoinType(relation.type);
      const methodName = relation.andSelect ? 'joinAndSelect' : 'join';
      queryBuilder[methodName](relation.property, relation.alias ?? getAlias(relation.property), relation.cond, joinType, relation.path);
    };
...
function getJoinType(type?: string): JoinType {
  switch (type) {
    case 'leftJoin':
