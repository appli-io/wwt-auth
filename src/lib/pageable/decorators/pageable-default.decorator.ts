...
import { ExtendedPageable, PageableQuery, Sort }              from '../types';
...
export const PageableDefault = createParamDecorator((data: PageableQuery, ctx: ExecutionContext): ExtendedPageable => {
  ...
  // Check if parsedSizeInt is a safe positive integer and less than or equal to maxSize
  const size =
    isSafePositiveInteger(parsedSizeInt) && parsedSizeInt <= maxSize
      ? parsedSizeInt // If true, assign parsedSizeInt to size
      : isSafePositiveInteger(defaultSize) && defaultSize <= maxSize // If not, check if defaultSize is a safe positive integer and less than or equal to maxSize
        ? defaultSize // If true, assign defaultSize to size
        : pageable.size <= maxSize // If not, check if pageable.size is less than or equal to maxSize
          ? pageable.size // If true, assign pageable.size to size
          : maxSize; // If all conditions fail, assign maxSize to size
